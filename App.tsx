    ...
    >
      <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-500">
        ...
        {activeTab === 'messages' ? (
          ...
          <>
            <section className="relative rounded-2xl sm:rounded-3xl overflow-hidden p-6 sm:p-12 text-white bg-slate-900 shadow-xl">
              <div className="absolute inset-0 opacity-10">
                 <img src="https://images.unsplash.com/photo-1523050853063-bd8012fbb761?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" alt="" />
              </div>
              <div className="relative z-10 max-w-xl">
                ...
                <p className="text-slate-400 text-xs sm:text-sm font-medium mb-6 uppercase tracking-widest leading-relaxed">
                  The official community portal for lost items.
                </p>
                <button onClick={handlePostReport} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 shadow-lg shadow-blue-500/20">
                  Post Report
                </button>
              </div>
            </section>
            ...
            <section className="sticky top-16 z-[60] flex flex-col sm:flex-row gap-2 bg-slate-50/80 backdrop-blur-md py-2 px-1">
              <div className="relative flex-grow">
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 focus:ring-4 focus:ring-blue-600/5 outline-none font-bold text-slate-700 text-xs transition-all"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
                <svg className="w-4 h-4 text-slate-400 absolute left-3.5 top-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>

              <div className="flex gap-2">
                <select className="flex-1 px-3 py-2.5 rounded-xl bg-white border border-slate-200 font-black text-[9px] uppercase tracking-wider text-slate-600 outline-none cursor-pointer" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)}>
                  <option value="ALL">All Status</option>
                  <option value={ItemStatus.LOST}>Lost</option>
                  <option value={ItemStatus.FOUND}>Found</option>
                </select>
                <select className="flex-1 px-3 py-2.5 rounded-xl bg-white border border-slate-200 font-black text-[9px] uppercase tracking-wider text-slate-600 outline-none cursor-pointer" value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {isLoading ? (
                ...
                <div className="col-span-full py-20 text-center">
                  <h3 className="text-xl font-black text-slate-300 uppercase">No Items Found</h3>
                </div>
              )}
            </div>
          </>
      ...
      <ItemDetailsModal item={selectedDetailItem} onClose={() => setSelectedDetailItem(null)} onMessage={(it) => { if(!currentUser) return setIsAuthModalOpen(true); setSelectedDetailItem(null); setSelectedMessageItem(it); }} />
    </Layout>
  );
};

const AuthRequiredState = ({ title, desc, onAuth }: { title: string, desc: string, onAuth: () => void }) => (
  <div className="p-8 text-center bg-white rounded-3xl border border-slate-100 shadow-sm max-w-sm mx-auto my-12">
    ...
    <button onClick={onAuth} className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">
      Sign In
    </button>
  </div>
);
