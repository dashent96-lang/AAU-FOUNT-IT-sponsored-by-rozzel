    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden flex h-[700px]">
      ...
      <div className={`${selectedConvId ? 'hidden lg:flex' : 'flex'} w-full lg:w-1/3 border-r border-slate-100 flex-col bg-slate-50/30`}>
        <div className="p-8 border-b border-slate-100">
          <h2 className="text-2xl font-black text-slate-800 tracking-tighter leading-none">Inbox</h2>
        </div>
        <div className="flex-grow overflow-y-auto p-4 space-y-2">
          {conversations.length === 0 ? (
            <div className="p-10 text-center opacity-40">
              <p className="text-xs font-black uppercase tracking-widest">No chats yet</p>
            </div>
            ...
            return (
              <button 
                ...
                className={`w-full text-left p-5 rounded-[2rem] transition-all flex items-center space-x-4 ${active ? 'bg-blue-600 text-white shadow-xl shadow-blue-200' : 'hover:bg-white text-slate-600'}`}
              >
                <div className={`w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center font-black ${active ? 'bg-white/20' : 'bg-blue-50 text-blue-600'}`}>
                ...
                <div className="min-w-0 flex-grow">
                  ...
                  <p className={`text-[10px] font-black uppercase tracking-widest truncate ${active ? 'text-blue-100' : 'text-slate-400'}`}>{conv.itemTitle}</p>
                </div>
              </button>
            );
          })}
      ...
      <div className={`${selectedConvId ? 'flex' : 'hidden lg:flex'} flex-grow flex-col`}>
        {activeConv ? (
          <>
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <button onClick={() => setSelectedConvId(null)} className="lg:hidden p-2 text-slate-400">
            ...
            <div className="flex-grow overflow-y-auto p-8 space-y-4 bg-slate-50/20">
              {activeConv.messages.map(m => (
                <div key={m.id} className={`flex ${m.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] px-5 py-3 rounded-[1.5rem] text-sm font-medium shadow-sm leading-relaxed ${
            ...
            <form onSubmit={handleSend} className="p-6 bg-white border-t border-slate-100">
              <div className="flex space-x-2 bg-slate-100 p-2 rounded-2xl">
                ...
                />
                <button type="submit" className="bg-blue-600 text-white p-4 rounded-xl shadow-lg hover:bg-blue-700 transition-all active:scale-95">
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
            ...
            <p className="font-black uppercase tracking-[0.2em] text-[10px]">Select a chat to begin recovery</p>
          </div>
        )}
      </div>
    </div>
