  ...
  return (
    ...
    >
      <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
        {activeTab === 'messages' ? (
          currentUser ? <MessagesView currentUser={currentUser} /> : (
          ...
          <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 lg:p-6 glass rounded-[2.5rem] border border-white/50 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] sticky top-28 z-40">
              ...
              <select className="px-6 py-4 rounded-2xl bg-white font-black outline-none border-none shadow-sm focus:ring-2 focus:ring-blue-500/10" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)}>
                <option value="ALL">All Status</option>
                <option value={ItemStatus.LOST}>Lost</option>
                <option value={ItemStatus.FOUND}>Found</option>
              </select>
              <select className="px-6 py-4 rounded-2xl bg-white font-black outline-none border-none shadow-sm focus:ring-2 focus:ring-blue-500/10" value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
              {isLoading ? (
                [1,2,3,4,5,6,7,8].map(i => <div key={i} className="h-[500px] bg-white rounded-[3rem] animate-pulse border border-slate-100" />)
              ) : filteredItems.length > 0 ? (
                filteredItems.map(item => (
                  <ItemCard 
                    key={item.id} 
                    item={item} 
                    onMessage={() => setSelectedMessageItem(item)} 
                    onViewDetails={() => setSelectedDetailItem(item)} 
                  />
                ))
              ) : (
                <div className="col-span-full py-24 text-center">
                  <h3 className="text-2xl font-black text-slate-400">No items found matching your criteria.</h3>
                </div>
              )}
            </div>
          </>
